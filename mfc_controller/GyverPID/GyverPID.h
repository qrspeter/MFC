...
public:
	...
	float Kd = 0.0;	
	float integral = 0.0;
	// Добавляем возможность ограничения интегральной составляющей
	void setIConstrain(datatype minI, datatype maxI);	// ограничение включается только если была вызвана функция setConstrain
...
private:
	...
	uint32_t pidTimer = 0;
	boolean _isiconstrain = 0;	// включать или нет ограничение интегрального члена. изменяется методом setIConstrain
	datatype _maxI, _minI;		// максимальные и минимальные значения интегрального члена
};
...
// включение ограничения интегрального члена
void GyverPID::setIConstrain(datatype minI, datatype maxI) {
	_isiconstrain = 1;
	_minI = minI;
	_maxI = maxI;
}
...
datatype GyverPID::getResult() {
	...
	// если надо ограничивать интегральный член то делаем это
	if (_isiconstrain)
		integral = constrain(integral, _minI, _maxI);	
